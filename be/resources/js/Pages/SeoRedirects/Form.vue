...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6 max-w-4xl">
            <!-- Validation Errors -->
            <div v-if="Object.keys(form.errors).length > 0" class="p-4 bg-rose-50 border border-rose-200 text-rose-800 rounded-xl text-sm font-medium">
                ...
                <form @submit.prevent="submit" class="p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col space-y-2 text-sm">
                            <label class="font-serif font-bold text-emerald-950">Đường dẫn cũ (Source URL) *</label>
                        ...
                        <!-- New URL -->
                        <div class="flex flex-col space-y-2 text-sm">
                            <label class="font-serif font-bold text-emerald-950">Đường dẫn mới (Target URL) *</label>
                        ...
                        <!-- HTTP Code -->
                        <div class="flex flex-col space-y-2 text-sm">
                            <label class="font-serif font-bold text-emerald-950">Mã chuyển hướng HTTP *</label>
                            <select v-model="form.http_code" required class="border border-zinc-200 rounded-lg px-4 py-2.5 bg-white text-zinc-950 focus:border-[#043616] focus:ring-1 focus:ring-[#043616] outline-none transition-all">
                        ...
                        <!-- Status -->
                        <div class="flex flex-col space-y-2 text-sm">
                            <label class="font-serif font-bold text-emerald-950">Trạng thái *</label>
                    ...
                    <div class="flex justify-end space-x-4 pt-4 border-t border-zinc-100">
                        <Link :href="route('admin.seo-redirects.index')" class="px-6 py-2.5 border border-zinc-200 text-zinc-600 rounded-lg text-sm font-semibold hover:bg-zinc-50 transition-all">Huỷ</Link>
                        <button type="submit" :disabled="form.processing" class="px-8 py-2.5 bg-[#043616] text-[#FFFDF9] rounded-lg text-sm font-semibold hover:bg-[#112215] transition-all disabled:opacity-50 hover:shadow-sm">Lưu Redirect</button>
                    </div>
                </form>
            </div>
        </div>
