...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
    <article v-for="item in media.data" :key="item.id" class="overflow-hidden rounded-lg border border-zinc-200 bg-white">
        <div class="aspect-video bg-zinc-50">
            <img :src="item.url" :alt="item.alt_text || item.original_name" class="h-full w-full object-cover"/>
        </div>
        <div class="space-y-3 p-4">
            <input v-model="item.alt_text" placeholder="Mô tả ảnh (alt text)" class="w-full rounded-lg border-zinc-200 text-sm"/>
            <div class="flex justify-between text-xs font-semibold">
                <button class="text-emerald-800" @click="saveAlt(item)">Lưu mô tả</button>
                <button class="text-zinc-600" @click="copy(item.url)">Sao chép URL</button>
                <button class="text-rose-600" @click="remove(item.id)">Xóa</button>
            </div>
        </div>
    </article>
</div>
